...
def getCleanList(ref, bases):
    ...
    try:
        for b in ibase_list:
            ...
            elif b in indels:  # Handle indels
                digits = []
                next_char = next(ibase_list, None)
                while next_char is not None and next_char.isdigit():
                    digits.append(next_char)
                    next_char = next(ibase_list, None)
                skip = int("".join(digits))
                if digits == 1:
                    pass
                else:
                    for _ in range(skip-1):
                        next(ibase_list, None)
            elif b == '^':  # Skip read qual noted at end of read
                next(ibase_list, None)
    except StopIteration:
        print("Reached end of input while processing indels.")
