...
def base_to_reads(df):
    df = df.copy()

    df["Allocated_Reads"] = np.where(
        df["Allocated_Bases"] >= df["Base_Count"],
        df["Read_Count"],
        np.maximum(
            1,
            np.round(
...
parser.add_argument('-l', '--min_link', type=float, default=0.99, help="Link (do not subsample) if asked to sample more than this percentage")
...
if args.manual_counts:
    ...
    expected_cols = ['Sample_ID', 'Group_0', 'Group_1', 'Read_Count','Base_Count', 'Forward', 'Reverse']
...
request_df = request_df[["Sample_ID","Group_0","Group_1","Final_Group","Read_Count","Base_Count","Requested_Bases","Forward","Reverse"]].copy()
balanced_df = balance_bases(request_df)

# Convert bases to reads
read_df = base_to_reads(balanced_df)

# Handle pairs
read_df["Allocated_Reads"] = np.where(
    read_df["Reverse"].notna(),
    np.ceil(read_df["Allocated_Reads"] / 2).astype(int),
    read_df["Allocated_Reads"]
)

# Linking
read_df["Sample_Type"] = np.where(
read_df["Allocated_Reads"] > args.min_link * read_df["Read_Count"],
"Link",
"Sample"
)

read_df["Subsample_ID"] = [uuid.uuid4().hex[:12] for _ in range(len(read_df))]    

summary_group0 = (
    read_df.groupby("Group_0")[["Allocated_Bases", "Base_Count"]]
    .sum()
    .reset_index()
)
...
summary_final = (
    read_df.groupby(["Group_0", "Final_Group"])[["Allocated_Bases", "Base_Count"]]
    .sum()
    .reset_index()
)
...
summary_samples = (
    read_df.groupby(["Group_0", "Final_Group", "Sample_ID", "Forward"])[["Allocated_Bases", "Base_Count"]]
    .sum()
    .reset_index()
)
...
read_df.to_csv(allocation_file,index=False,sep="\t")
read_df['Output_Dir'] = os.path.abspath(args.output)
read_df[['Sample_ID','Subsample_ID','Forward','Reverse','Allocated_Reads','Sample_Type','Output_Dir']].to_csv(sys.stdout, index=False, header=False)