...
if(not os.path.isdir(os.path.dirname(os.path.abspath(sys.argv[1]))+'/Whole_Genome_Mapping')):
    os.mkdir(os.path.dirname(os.path.abspath(sys.argv[1]))+'/Whole_Genome_Mapping')
...
allData = pd.read_csv(output_dir + "/WholeGenome_" + genomeName + "_Mapped.bed",header=None,dtype={0: object, 1: int, 2: int, 3: object},sep="\t")
...
nonDupData.to_csv(output_dir + "/WholeGenome_" + genomeName + "_Mapped_NonDup.bed",sep='\t',header=False,index=False)
...
print("A genome map file for " + genomeName + " without overlapping sites (" + "{:,}".format(nonDupSites) + " sites) was created at " + output_dir  + "/WholeGenome" + genomeName + "_Mapped_NonDup.bed\n")
